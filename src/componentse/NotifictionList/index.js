...
function NotificationList({ data, changeStreen = () => {} }) {
  ...
  return (
    <>
      ...
      {newItem.map((item) => (
        <div
          className="dark-notification-list"
          onDoubleClick={() => changeStreen("/read")}
        >
          <div className="dark-list-item1">
            <input
              type="checkbox"
              onChange={(e) => onSelectItems(item.id, e.currentTarget.checked)}
            />
          </div>
          ...
          <div className="dark-list-item3">{item.content}</div>
          <div className="dark-list-item4">
